...
public class Exercise_06_26 {
	...
	public static void main(String[] args) {
		...
		int index = 2;	// Possible palindromic prime numbers

		// Display 10 numbers per line, separated by exactly one space
		while (count < NUMBER_OF_PALINDROMIC_PRIMES) {
			if (PrimeNumberMethod.isPrime(index) && 
				 Exercise_06_03.isPalindrome(index)) {
				count++; // Increment count
			
				System.out.print(count % NUMBERS_PER_LINE == 0 ? 
					index + "\n" : index + " ");
			}
